...
fn render(frame: &mut Frame, app: &App) {
    ...
    let world = Paragraph::new(build_world_lines(app, Instant::now()))
        .block(
            Block::default()
                .title(format!(
                    " World {}x{} | Tick {} ",
...
fn build_world_lines(app: &App, now: Instant) -> Vec<Line<'static>> {
        ...
        .map(|row| {
            let spans = row
                ...
                .map(|cell| match cell {
                    ...
                    CellKind::Feature(glyph, false) => {
                        Span::styled(glyph.to_string(), Style::default().fg(Color::Blue))
                    }
                    ...
                    CellKind::Player { glyph, is_local } if is_local => Span::styled(
                        glyph.to_string(),
                        Style::default()
                            .fg(Color::Cyan)
                            .add_modifier(Modifier::BOLD),
                    ),
                    CellKind::Player { glyph, is_local: _ } => Span::styled(
                        glyph.to_string(),
                        Style::default()
                            .fg(Color::Yellow)
                            .add_modifier(Modifier::BOLD),
    ...
    app.chat_log[start..]
        ...
        .map(|message| {
            let prefix = match message.kind {
                ChatKind::Player => format!("{}:", message.from),
                ChatKind::System => "system:".to_string(),
            };
            let style = match message.kind {
                ChatKind::Player => Style::default().fg(Color::White),
                ChatKind::System => Style::default().fg(Color::Blue),
            };
            Line::from(vec![
...
enum CellKind {
    ...
    Feature(char, bool),
    Collectible(char),
    Player { glyph: char, is_local: bool },
}
